...
(function($){
	...
	handler = {
		onSwitchNode: function (event, node) {
			if (node != null && node.disable) return;
	...
	view = {
		...
		createNodes: function(setting, level, nodes, parentNode) {
			...
			// 根据属性selected选择相应节点  -----bengin-----
			var results = [], disableResults = [], disabled = true;
			view.getTreeSelectedNodes(setting, nodes, true, results);
            for (var i=0, l=results.length; i<l; i++) {
				view.selectNode(setting, results[i], true);
			}
			
			// 根据disable值设置节点状态
			view.getTreeDisableNodes(setting, nodes, true, disableResults);
			for (var j=0, l=disableResults.length; j<l; j++) {
				view.disableNode(setting, disableResults[j]);
			}
			// -----end-----
		...
		// 获取属性selected值为true的node -----begin-----
		getTreeSelectedNodes: function(setting, nodes, selected, results) {
			if (!nodes) return [];
			var childKey = setting.data.key.children,
			selectedKey = setting.data.key.selected,
			results = !results ? [] : results;
			for (var i = 0, l = nodes.length; i < l; i++) {
				if (nodes[i][selectedKey] == selected) {
					results.push(nodes[i]);
				}
				view.getTreeSelectedNodes(setting, nodes[i][childKey], selected, results);
			}
			return results;
		},
		
		getTreeDisableNodes: function(setting, nodes, disable, results) {
			if (!nodes) return [];
			var childKey = setting.data.key.children,
			disableKey = setting.data.key.disable,
			results = !results ? [] : results;
			for (var i = 0, l = nodes.length; i < l; i++) {
				if (nodes[i][disableKey] == disable) {
					results.push(nodes[i]);
				}
				view.getTreeDisableNodes(setting, nodes[i][childKey], disable, results);
			}
			return results;
		},
		
		disableNode: function(setting, node) {
			$$(node, consts.id.A, setting).addClass(consts.node.DISABLENODE);
			view.repairSonChkDisabled(setting, node, true, false);
			view.repairParentChkDisabled(setting, node.getParentNode(), false, false);
		},
