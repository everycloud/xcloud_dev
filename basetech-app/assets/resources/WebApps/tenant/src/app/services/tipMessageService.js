define(["tiny-lib/jquery", "tiny-lib/encoder", "language/keyID"], function ($, $encoder, i18n) {
    "use strict";
    $.fn.alert = function () {
        var self = $(this);
    ...
    var tipMessage = function ($timeout) {
        ...
        this.alert = function (type, message, extra_tags) {
            ...
            if (type === "danger") {
                ...
                messageTemplate.find("strong").text((i18n.common_term_danger_label||"危险")+":");
            }
            if (type === "error") {
                messageTemplate.addClass("alert-error");
                messageTemplate.find("strong").text((i18n.common_term_fail_label||"失败")+":");
            }
            if (type === "success") {
                messageTemplate.addClass("alert-success");
                messageTemplate.find("strong").text((i18n.common_term_success_value||"成功")+":");
            }
            if (type === "warning") {
                messageTemplate.addClass("alert-warning");
                messageTemplate.find("strong").text((i18n.common_term_warning_label||"警告")+":");
            }
            if (type === "info") {
                messageTemplate.addClass("alert-info");
                messageTemplate.find("strong").text((i18n.common_term_tip_label||"提示")+":");
            }
        ...
        this.sspAlert = function (orderId, clickFn) {
            var messageTemplate = $('<div class="alert alert-block fade in"><a class="close" data-dismiss="alert">&times;</a><p><strong></strong></p>' + i18n.service_order_commit_info_succees_msg + '<a style="cursor:pointer" class="orderClick"></a></div>');
            messageTemplate.addClass("alert-success");
            messageTemplate.find("strong").text((i18n.common_term_success_value||"成功")+":");

            messageTemplate.find(".orderClick").text(orderId);
            messageTemplate.find(".orderClick").bind("click", clickFn);
            messageTemplate.alert();
            messageTemplate.hide().prependTo("#service-content .messages").fadeIn(100);
            this.autoDismissAlert(messageTemplate);
            return messageTemplate;
