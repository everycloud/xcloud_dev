...
function LanguageService()
{
	...
	{	return ( a_strOp == "and"  || a_strOp == "&");	}
	this.isOR = function( a_strOp )
	{	return ( a_strOp == "or"  || a_strOp == "|" );		}
	this.isNOT = function( a_strOp )
	{	return ( a_strOp == "not"  || a_strOp == "~" );	}
	...
	if( !gbReady )
	{
		...
		if( SendMessage( oResMsg ) && oResMsg.oParam )
		{
			...
			if( aProj && aProj.length > 0 )
			{
				...
				{
				    if(
				    (typeof(aProj[i].sPPath)!='undefined')
				       &&(typeof(aProj[i].sDPath)!='undefined')
				       && (typeof(aProj[i].sFts)!='undefined')
				    )
					{
					    goOdinHunter.aProjPathes[tmpindex] = new Object();
					    goOdinHunter.aProjPathes[tmpindex].strProjDir = aProj[i].sPPath;
					    goOdinHunter.aProjPathes[tmpindex].strOdbPath = aProj[i].sPPath + aProj[i].sDPath + aProj[i].sFts;
					    tmpindex++;
					}
				}
