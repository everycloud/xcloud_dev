define(["can/util/fixture/fixture"], function (fixture) {
    ...
    //vxlan组播IP地址池
    var multicastIpPoolsList = {
        "multicastIPPools":[
            {
                "multicastIPPoolID":"1000000",
                "name":"FMVXIP_SZA01",
                "description":null,
                "startIP":"224.0.1.0",
                "endIP":"224.3.1.255",
    ...
    fixture({
        ...
        "GET /goku/rest/v1.5/irm/zone/46161896180547584010755/multicast-ippools": function (original, response) {
            ...
            for(var i=0; i<multicastIpPoolsList.multicastIPPools.length; i++){
                array[i] = multicastIpPoolsList.multicastIPPools[i];
            }
            var result = {
                multicastIPPools:array
            };
        ...
        "POST /goku/rest/v1.5/irm/multicast-ippools": function (original, response) {
            ...
            var vxlanIPPool = {
                multicastIPPoolID: Math.ceil(Math.random()*100000) + "",
                name: request.name,
                description: request.description,
                startIP: request.startIP,
                endIP: request.endIP,
                totalUsedIPs:0,
                totalIPs: Math.ceil(Math.random()*10000)
            };
            multicastIpPoolsList.multicastIPPools.push(vxlanIPPool);
