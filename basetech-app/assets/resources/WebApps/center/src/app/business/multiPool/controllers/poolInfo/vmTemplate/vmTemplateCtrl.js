...
define(["tiny-lib/jquery",
    ...
    function ($, angular, _, $encoder, Window, CloudInfraService, TemplateService) {
        "use strict";
        var vmTemplateListCtrl = ["$rootScope", "$scope", "$compile", "$state", "$q", "camel", "exception", "$stateParams",
            function ($rootScope, $scope, $compile, $state, $q, camel, exception, $stateParams) {
                //查询资源池信息的服务实例
                var cloudInfraServiceIns = new CloudInfraService($q, camel);
                var templateServiceIns = new TemplateService(exception, $q, camel);
                var cloudInfraId = $stateParams.infraId;
                var user = $scope.user;
                var i18n = $scope.i18n;
                //当前页码信息
                var page = {
                ...
                $scope.openstack = ($rootScope.user.cloudType === "ICT");
                // 权限控制
                //虚拟机模板
                ...
                $scope.versionConfig = {
                    ...
                    "getVersionInfo": function (osType) {
                        var linuxVersions = $scope.versionConfig.Linux.split(";");
                        var windowsVersions = $scope.versionConfig.windows.split(";");
                        var versionList = [];
                        var versions = [
                            {
                                "selectId": "",
                        ...
                        if (osType === 'Windows') {
                            versionList = windowsVersions;
                        } else if (osType === 'Linux') {
                            versionList = linuxVersions;
                        } else if (osType === "Other") {
                            versions.push(otherVersion);
                        } else {
                            versionList = linuxVersions.concat(windowsVersions);
                        }
                        ...
                        if (osType !== "Other") {
                            ...
                            _.each(versionList, function (item) {
                                key = item.substr(0, item.lastIndexOf("_"));
                                version = {
                                    "selectId": key,
                                    "label": key
                                };
                    ...
                    promise.then(function (data) {
                        _.each(data.vmtemplates, function (item) {
                            if (!$scope.openstack) {
                                item.detail = {
                                    contentType: "url",
                                    content: "app/business/multiPool/views/poolInfo/vmTemplate/vmTemplateDetail.html"
                                };
                                if (item.vdcId === "1") {
                ...
                function init() {
                    ...
                    $scope.searchOSVersion.values = $scope.versionConfig.getVersionInfo("");
                    queryTemplateList();
                }
