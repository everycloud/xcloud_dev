    ...
    function ($, angular, Checkbox, Radio, Select, Textbox, Button, Window, Message, UnifyValid, validatorService, catalogService, createService) {
        ...
        var createStorageCtrl = ["$scope", "$compile", "$state", "$stateParams", "camel", "$q", "validator", "exception", function ($scope, $compile, $state, $stateParams, camel, $q, validator, exception) {
            ...
            $scope.createStorageStep1 = {
                "url": "../src/app/business/service/views/catalog/storage/locationInfo.html"
            };
            $scope.createStorageStep2 = {
                "url": "../src/app/business/service/views/catalog/storage/storageQuotaInfo.html"
            };
            $scope.createStorageStep3 = {
                "url": "../src/app/business/service/views/catalog/storage/basicInfo.html"
            };
            $scope.createStorageStep4 = {
                "url": "../src/app/business/service/views/catalog/storage/createStorageConfirmInfo.html"
            };
            //控制步骤中各页面的显示
            ...
            $scope.locationInfo = {
                ...
                nextBtn: {
                    ...
                    "click": function () {
                        ...
                        if($scope.cloudType == "IT"){
                            var storageConfigStatus = $("#" + $scope.disk.storageConfigLock.id).widget();
                        }
                        ...
                        if (appTypeStatus === "none") {
                            ...
                            if($scope.cloudType == "IT"){
                                storageConfigStatus.opDisabled("2", true);
                            }
                        }
                        else {
                            ...
                            if($scope.cloudType == "IT"){
                                storageConfigStatus.opDisabled("2", false);
                            }
                        }
                        if (areaLock === "0") {
                            ...
                            storageMediaStatus.opChecked(serviceId?$scope.diskMedialockId:areaLock, true);
                        } else if (areaLock === "1") {
                            ...
                            if (oldMedia == "0") {
                                storageMediaStatus.opChecked(serviceId?$scope.diskMedialockId:"2", true);
                            }
                        }
                        else {
                            storageMediaStatus.opDisabled("0", true);
                        ...
                        if(serviceId && $scope.cloudType == "IT"){
                            storageConfigStatus.opChecked($scope.modifyStorageConfiglockID,true);
                        }
                        if($scope.cloudType == "IT"){
            ...
            function initParams(serviceInstance) {
                ...
                if($scope.cloudType == "IT"){
                    ...
                    $scope.modifyStorageConfiglockID = serviceInstaceQuota.configType.lock;
                }
                ...
                $scope.diskMedialockId = serviceInstaceQuota.mediaType.lock;
                $scope.disk.diskTypeLock.change();
                $scope.disk.diskSizeLock.change();
                $scope.disk.diskMediaLock.change();
                if($scope.cloudType == "IT"){
