        ...
        var subnetCtrl = ["$scope", "$state", "validator", "camel", "$rootScope", function ($scope, $state, validator, camel, $rootScope) {
                ...
                $scope.nextBtn = {
                    ...
                    "click": function () {
                        ...
                        // 触发事件
                        $scope.$emit($scope.createExtNetworkEvents.subnetComplete);
                    }
                ...
                $scope.$on($scope.createExtNetworkEvents.initSubnet, function (event, msg) {
                    if (null != $scope.connectInfo.ipv4Subnet) {
                        $("#" + $scope.ipv4.ipv4Checkbox.id).widget().option("checked", true);
                        if ($scope.connectInfo.ipv4Subnet.usedAddrNum > 0) {
                            setTimeout(function () {
                                $("#" + $scope.ipv4.ipv4Checkbox.id).widget().option("disable", true);
                            }, 100);
                        }

                        isIpv4Checked = true;
                        $("#" + $scope.ipv4.ipDistMode.id).widget().opChecked($scope.connectInfo.ipv4Subnet.ipAllocatePolicy.toString(), true);
                        if ($scope.connectInfo.totalBoundNics > 0) {
                            setTimeout(function () {
                                $("#" + $scope.ipv4.ipDistMode.id).widget().opDisabled("0", true);
                                $("#" + $scope.ipv4.ipDistMode.id).widget().opDisabled("1", true);
                                $("#" + $scope.ipv4.ipDistMode.id).widget().opDisabled("2", true);
                                $("#" + $scope.ipv4.ipDistMode.id).widget().opDisabled("3", true);
                            }, 100);
                        }
                        $("#" + $scope.ipv4.subnetIp.id).widget().option("value", $scope.connectInfo.ipv4Subnet.subnetAddr);
                        $("#" + $scope.ipv4.subnetMask.id).widget().option("value", $scope.connectInfo.ipv4Subnet.subnetPrefix);
                        $("#" + $scope.ipv4.gateway.id).widget().option("value", $scope.connectInfo.ipv4Subnet.gateway);
                        $scope.service.ipv4DistMode = $scope.ipv4.ipDist;
                        switch ($("#" + $scope.ipv4.ipDistMode.id).widget().opChecked("checked")) {
                            case constants.config.IP_ALLOCATE_POLICY.external:
                                $("#" + $scope.ipv4.dhcpIp01.id).widget().option("value", $scope.connectInfo.ipv4Subnet.dhcpOption.dhcpServerIP1);
                                $("#" + $scope.ipv4.dhcpIp02.id).widget().option("value", $scope.connectInfo.ipv4Subnet.dhcpOption.dhcpServerIP2);
                                $scope.ipv4.ipDist = "ext";
                                break;
                            case constants.config.IP_ALLOCATE_POLICY.internal:
                                $scope.ipv4.ipDist = "in"
                                for (var index in $scope.connectInfo.ipv4Subnet.availableIPRanges) {
                                    var id = "availableIpSegment" + (parseInt(index) + 1);
                                    $scope.ipv4[id].display = true;
                                    $("#" + $scope.ipv4[id].startIp.id).widget().option("value", $scope.connectInfo.ipv4Subnet.availableIPRanges[index].startIP);
                                    $("#" + $scope.ipv4[id].endIp.id).widget().option("value", $scope.connectInfo.ipv4Subnet.availableIPRanges[index].endIP);
                                }
                                $("#" + $scope.ipv4.domain.id).widget().option("value", $scope.connectInfo.ipv4Subnet.dhcpOption.domainName);
                                $("#" + $scope.ipv4.firstDns.id).widget().option("value", $scope.connectInfo.ipv4Subnet.dhcpOption.primaryDNS);
                                $("#" + $scope.ipv4.alternativeDns.id).widget().option("value", $scope.connectInfo.ipv4Subnet.dhcpOption.secondaryDNS);
                                $("#" + $scope.ipv4.firstWins.id).widget().option("value", $scope.connectInfo.ipv4Subnet.dhcpOption.primaryWINS);
                                $("#" + $scope.ipv4.alternativeWins.id).widget().option("value", $scope.connectInfo.ipv4Subnet.dhcpOption.secondaryWINS);
                                if ($scope.connectInfo.ipv4Subnet.usedAddrNum > 0) {
                                    setTimeout(function () {
                                        $("#" + $scope.ipv4.ipDistMode.id).widget().opDisabled("0", true);
                                        $("#" + $scope.ipv4.ipDistMode.id).widget().opDisabled("1", true);
                            ...
                            case constants.config.IP_ALLOCATE_POLICY.staticInjection:
                                ...
                                if ($scope.connectInfo.ipv4Subnet.usedAddrNum > 0) {
                                    setTimeout(function () {
                                        ...
                                        $("#" + $scope.ipv4.ipDistMode.id).widget().opDisabled("2", true);
                                        $("#" + $scope.ipv4.ipDistMode.id).widget().opDisabled("3", true);
                                        $("#" + $scope.ipv4.subnetIp.id).widget().setDisable(true);
                                        $("#" + $scope.ipv4.subnetMask.id).widget().setDisable(true);
                                        $("#" + $scope.ipv4.gateway.id).widget().setDisable(true);
                                        $("#" + $scope.ipv4.domain.id).widget().option("disable", true);
                                        $("#" + $scope.ipv4.firstDns.id).widget().setDisable(true);
                                        $("#" + $scope.ipv4.alternativeDns.id).widget().setDisable(true);
                                        $("#" + $scope.ipv4.firstWins.id).widget().setDisable(true);
                                        $("#" + $scope.ipv4.alternativeWins.id).widget().setDisable(true);
                                    }, 100);
                                }
                                break;
                            case constants.config.IP_ALLOCATE_POLICY.manual:
                        ...
                        isIpv4Checked = false;
                        $("#" + $scope.ipv4.ipv4Checkbox.id).widget().option("checked", false);
                    }
                    $scope.operate.setIpv4DivStatus(isIpv4Checked);
                    if (null != $scope.connectInfo.ipv6Subnet) {
                        $("#" + $scope.ipv6.ipv6Checkbox.id).widget().option("checked", true);
                        if ($scope.connectInfo.ipv6Subnet.usedAddrNum > 0) {
                            setTimeout(function () {
                                $("#" + $scope.ipv6.ipv6Checkbox.id).widget().option("disable", true);
                            }, 100);
                        }
                        isIpv6Checked = true;
                        $("#" + $scope.ipv6.ipDistMode.id).widget().opChecked($scope.connectInfo.ipv6Subnet.ipAllocatePolicy.toString(), true);
                        if ($scope.connectInfo.totalBoundNics > 0) {
                            setTimeout(function () {
                                $("#" + $scope.ipv6.ipDistMode.id).widget().opDisabled("0", true);
                                $("#" + $scope.ipv6.ipDistMode.id).widget().opDisabled("3", true);
                                $("#" + $scope.ipv6.ipDistMode.id).widget().opDisabled("4", true);
                            }, 100);
                        }
                        $("#" + $scope.ipv6.subnetIp.id).widget().option("value", $scope.connectInfo.ipv6Subnet.subnetAddr);
                        $("#" + $scope.ipv6.subnetPrefix.id).widget().option("value", $scope.connectInfo.ipv6Subnet.subnetPrefix);
                        $("#" + $scope.ipv6.gateway.id).widget().option("value", $scope.connectInfo.ipv6Subnet.gateway);
                        $scope.service.ipv6DistMode = $scope.ipv6.ipDist;
                        switch ($("#" + $scope.ipv6.ipDistMode.id).widget().opChecked("checked")) {
                            case constants.config.IP_ALLOCATE_POLICY.external:
                                $scope.ipv6.ipDist = "ext"
                                $("#" + $scope.ipv6.dhcpIp01.id).widget().option("value", $scope.connectInfo.ipv6Subnet.dhcpOption.dhcpServerIP1);
                                $("#" + $scope.ipv6.dhcpIp02.id).widget().option("value", $scope.connectInfo.ipv6Subnet.dhcpOption.dhcpServerIP2);
                                break;
                            case constants.config.IP_ALLOCATE_POLICY.staticInjection:
                                $scope.ipv6.ipDist = "static"
                                for (var index in $scope.connectInfo.ipv6Subnet.availableIPRanges) {
                                    var id = "availableIpSegment" + (parseInt(index) + 1);
                                    $scope.ipv6[id].display = true;
                                    $("#" + $scope.ipv6[id].startIp.id).widget().option("value", $scope.connectInfo.ipv6Subnet.availableIPRanges[index].startIP);
                                    $("#" + $scope.ipv6[id].endIp.id).widget().option("value", $scope.connectInfo.ipv6Subnet.availableIPRanges[index].endIP);
                                }
                                $("#" + $scope.ipv6.domain.id).widget().option("value", $scope.connectInfo.ipv6Subnet.dhcpOption.domainName);
                                $("#" + $scope.ipv6.firstDns.id).widget().option("value", $scope.connectInfo.ipv6Subnet.dhcpOption.primaryDNS);
                                $("#" + $scope.ipv6.alternativeDns.id).widget().option("value", $scope.connectInfo.ipv6Subnet.dhcpOption.secondaryDNS);
                                if ($scope.connectInfo.ipv6Subnet.usedAddrNum > 0) {
                                    setTimeout(function () {
                                        $("#" + $scope.ipv6.ipDistMode.id).widget().opDisabled("0", true);
                                        $("#" + $scope.ipv6.ipDistMode.id).widget().opDisabled("3", true);
                                        $("#" + $scope.ipv6.ipDistMode.id).widget().opDisabled("4", true);
                                        $("#" + $scope.ipv6.subnetIp.id).widget().option("disable", true);
                                        $("#" + $scope.ipv6.subnetPrefix.id).widget().option("disable", true);
                                        $("#" + $scope.ipv6.gateway.id).widget().option("disable", true);
                                        $("#" + $scope.ipv6.domain.id).widget().option("disable", true);
                                        $("#" + $scope.ipv6.firstDns.id).widget().option("disable", true);
                                        $("#" + $scope.ipv6.alternativeDns.id).widget().option("disable", true);
                                    }, 100);
                                }
                                break;
                            case constants.config.IP_ALLOCATE_POLICY.autoLayout:
                                $scope.ipv6.ipDist = "auto"
                                break;
                            default:
                                break;
                        ...
                        isIpv6Checked = false;
                        $("#" + $scope.ipv6.ipv6Checkbox.id).widget().option("checked", false);
                    }
                    $scope.operate.setIpv6DivStatus(isIpv6Checked);
                });
            }]
