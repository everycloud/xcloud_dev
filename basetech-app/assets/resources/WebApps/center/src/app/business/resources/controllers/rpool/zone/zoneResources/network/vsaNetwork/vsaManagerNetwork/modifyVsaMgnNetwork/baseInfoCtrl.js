...
define(["tiny-lib/angular",
    ...
    function (angular, UnifyValid, constants) {
        ...
        var baseInfoCtrl = ["$scope", "$state", "$stateParams", "validator", function ($scope, $state, $stateParams, validator) {
            ...
            $scope.service.vsaManageNetworkID = $stateParams.id;
            $scope.service.init($scope.service.vsaManageNetworkID);
            $scope.name = {
                "id": "nameText",
            ...
            $scope.$on($scope.modifyVsaMgnNetworkEvents.setConnectInfo, function (event, msg) {
                setTimeout(function () {
                    $("#" + $scope.name.id).widget().option("value", $scope.connectInfo.name);
                    $("#" + $scope.description.id).widget().option("value", $scope.connectInfo.description);
                }, 100);
            });
        }];
