...
define(["tiny-lib/angular",
    ...
    function (angular, Checkbox, UnifyValid, constants, ExceptionService) {
        "use strict";
        var modifyVtepNetworkCtrl = ["$scope", "$state", "$stateParams", "$rootScope", "validator", "camel", function ($scope, $state, $stateParams, $rootScope, validator, camel) {
            $scope.action = "edit";
            $scope.zoneInfo = {
            ...
            $scope.basicInfo = {
                ...
                nextBtn: {
                    ...
                    "click": function () {
                        ...
                        setTimeout(function () {
                            $("#" + $scope.resource.subnetIp.id).widget().option("value", $scope.vtepNetworkInfo.subnetIP);
                            $("#" + $scope.resource.subnetMask.id).widget().option("value", $scope.vtepNetworkInfo.subnetMask);
                            $("#" + $scope.resource.gateway.id).widget().option("value", $scope.vtepNetworkInfo.subnetGateway);
                            $("#" + $scope.resource.vlan.id).widget().option("value", $scope.vtepNetworkInfo.vlan);
                            for (var index in $scope.vtepNetworkInfo.availableIPRanges) {
                                var id = "availableIpSegment" + (parseInt(index) + 1);
                                $scope.resource[id].display = true;
                                $("#" + $scope.resource[id].startIp.id).widget().option("value", $scope.vtepNetworkInfo.availableIPRanges[index].startIP);
                                $("#" + $scope.resource[id].endIp.id).widget().option("value", $scope.vtepNetworkInfo.availableIPRanges[index].endIP);
                            }
                        }, 100);
                ...
                dvsTable: {
                    ...
                    "renderRow": function (nRow, aData, iDataIndex) {
                        ...
                        var options = {
                            ...
                            "checked": $.inArray(aData.name, $scope.vtepNetworkInfo.dvsNames) >= 0,
                            "disable": $scope.vtepNetworkInfo.usedFlag && $.inArray(aData.name, $scope.vtepNetworkInfo.dvsNames) >= 0,
                            "change": function () {
                ...
                createBtn: {
                    ...
                    "click": function () {
                        var modifyInfo = {
                            ...
                            "description": $scope.createInfo.description,
                            "availableIPRanges": $scope.createInfo.availableIPRanges,
                            "dvsIDs": $scope.createInfo.dvsIDs
                        };
                        $scope.operate.modify(modifyInfo);
                    }
                },
                cancelBtn: {
                    "id": "cancelBtn",
            ...
            $scope.operate = {
                ...
                "modify": function (params) {
                    var modifyConfig = constants.rest.VTEP_UPDATE;
                ...
                "queryDetailInfo": function (zoneid, id) {
                    ...
                    deferred.done(function (response) {
                        $scope.$apply(function () {
                            $scope.vtepNetworkInfo = response;
                            var dvses = $scope.vtepNetworkInfo.dvses;
                            var dvsNames = [];
                            for (var index in dvses) {
                                dvsNames.push(dvses[index].name);
                            }
                            $scope.vtepNetworkInfo.dvsNames = dvsNames;
                        });
                        setTimeout(function () {
                            $("#" + $scope.basicInfo.name.id).widget().option("value", $scope.vtepNetworkInfo.name);
                            $("#" + $scope.basicInfo.description.id).widget().option("value", $scope.vtepNetworkInfo.description)
                        }, 100);
                    });
                    deferred.fail(function (response) {
                        new ExceptionService().doException(response);
            ...
            $scope.operate.queryDetailInfo($scope.zoneInfo.id, id);
        }];

        return modifyVtepNetworkCtrl;
    })
;
