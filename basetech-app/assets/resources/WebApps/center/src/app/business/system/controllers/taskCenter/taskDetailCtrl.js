        ...
        var taskDetailCtrl = ["$scope", "camel", "$compile", function ($scope, camel, $compile) {
            ...
            function setTaskInfo(data) {
                ...
                if (mdata == null) {
                    ...
                    for (var i = 0; i < len; i++) {
                        if (mdata[i].taskProgress.status == 'FAILED') {
                            ++j;
                        } else if (mdata[i].taskProgress.status == 'SUCCESS') {
                            ++k;
                        } else if (mdata[i].taskProgress.status == 'RUNNING') {
                            ++m;
                        }
                        mdata[i].statusStr = constants.config.TASK_STATUS[mdata[i].taskProgress.status];
                        mdata[i].taskEventtype.eventType = TaskMap[mdata[i].taskEventtype.eventTypeString.toString()] || mdata[i].taskEventtype.eventTypeString;
                        mdata[i].SequenceNum = i + 1;

                        var taskTime = mdata[i].taskTime;
            ...
            $scope.getTaskDetail = function () {
                var defered = camel.get({
                    "url": {
                        s: "/goku/rest/v1.5/1/tasks?taskid={taskid}&withsubtask={withsubtask}",
                        o: {"taskid": $scope.taskId, "withsubtask": 'true'}
                    },
                    "userId": user.id
                });
