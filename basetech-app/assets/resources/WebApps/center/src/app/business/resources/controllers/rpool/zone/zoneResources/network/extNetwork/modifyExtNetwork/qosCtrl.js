...
define(["tiny-lib/angular",
    ...
    function (angular, UnifyValid, constants) {
        ...
        var qosCtrl = ["$scope", "$state", function ($scope, $state) {
            ...
            $scope.arpMsgSuppressionCheckbox = {
                ...
                "change": function () {
                    if ($("#" + $scope.arpMsgSuppressionCheckbox.id).widget().option("checked")) {
                        $("#" + $scope.arpMsgSuppressionTextbox.id).widget().option("disable", false);
                    }
                    else {
                        $("#" + $scope.arpMsgSuppressionTextbox.id).widget().option("disable", true);
                    }
            ...
            $scope.ipMsgSuppressionCheckbox = {
                ...
                "change": function () {
                    if ($("#" + $scope.ipMsgSuppressionCheckbox.id).widget().option("checked")) {
                        $("#" + $scope.ipMsgSuppressionTextbox.id).widget().option("disable", false);
                    }
                    else {
                        $("#" + $scope.ipMsgSuppressionTextbox.id).widget().option("disable", true);
                    }
            ...
            $scope.nextBtn = {
                ...
                "click": function () {
                    ...
                    if ("1" == $("#" + $scope.sendSpeedLimit.id).widget().opChecked("checked")) {
                        ...
                        if (!TxTrafficShapingPolicy.priority) {
                            TxTrafficShapingPolicy.priority = "";
                            $scope.service.priority = "";
                        }
                        else {
            ...
            // 事件处理
            $scope.$on($scope.createExtNetworkEvents.initQos, function (event, msg) {
                if ($scope.connectInfo.portSetting.outTrafficShapingPolicyFlag) {
                    $("#" + $scope.sendSpeedLimit.id).widget().opChecked("1", true);
                    $("#" + $scope.priority.id).widget().opDisabled("2", false);
                    $("#" + $scope.priority.id).widget().opDisabled("4", false);
                    $("#" + $scope.priority.id).widget().opDisabled("7", false);
                    $scope.sendAvgBandwidth.disable = false;
                    $scope.sendPeakBandwidth.disable = false;
                    $scope.sendBurstSize.disable = false;
                    $("#" + $scope.sendAvgBandwidth.id).widget().option("value", $scope.connectInfo.portSetting.outTrafficShapingPolicy.averageBandwidth);
                    $("#" + $scope.sendPeakBandwidth.id).widget().option("value", $scope.connectInfo.portSetting.outTrafficShapingPolicy.peakBandwidth);
                    $("#" + $scope.sendBurstSize.id).widget().option("value", $scope.connectInfo.portSetting.outTrafficShapingPolicy.burstSize);
                    $("#" + $scope.priority.id).widget().opChecked($scope.connectInfo.portSetting.outTrafficShapingPolicy.priority.toString(), true)
                }
                if ($scope.connectInfo.portSetting.inTrafficShapingPolicyFlag) {
                    $("#" + $scope.receiveSpeedLimit.id).widget().opChecked("1", true);
                    $scope.receiveAvgBandwidth.disable = false;
                    $scope.receivePeakBandwidth.disable = false;
                    $scope.receiveBurstSize.disable = false;
                    $("#" + $scope.receiveAvgBandwidth.id).widget().option("value", $scope.connectInfo.portSetting.inTrafficShapingPolicy.averageBandwidth);
                    $("#" + $scope.receivePeakBandwidth.id).widget().option("value", $scope.connectInfo.portSetting.inTrafficShapingPolicy.peakBandwidth);
                    $("#" + $scope.receiveBurstSize.id).widget().option("value", $scope.connectInfo.portSetting.inTrafficShapingPolicy.burstSize);
                }
                if (0 != $scope.connectInfo.portSetting.arpPacketSuppression) {
                    $("#" + $scope.arpMsgSuppressionCheckbox.id).widget().option("checked", true);
                    $("#" + $scope.arpMsgSuppressionTextbox.id).widget().option("disable", false);
                    $("#" + $scope.arpMsgSuppressionTextbox.id).widget().option("value", $scope.connectInfo.portSetting.arpPacketSuppression);
                }
                if (0 != $scope.connectInfo.portSetting.ipPacketSuppression) {
                    $("#" + $scope.ipMsgSuppressionCheckbox.id).widget().option("checked", true);
                    $("#" + $scope.ipMsgSuppressionTextbox.id).widget().option("disable", false);
                    $("#" + $scope.ipMsgSuppressionTextbox.id).widget().option("value", $scope.connectInfo.portSetting.ipPacketSuppression);
                }
                $("#" + $scope.dhcpIsolationCheckbox.id).widget().option("checked", $scope.connectInfo.portSetting.dhcpIsolationFlag);
                $("#" + $scope.ipMacBindingCheckbox.id).widget().option("checked", $scope.connectInfo.portSetting.ipMacBindFlag);
            });
        }];
