...
define(['tiny-lib/jquery',
    ...
    function ($, sprintf, ngSanitize, keyIDI18n, angular, http, constants, Window, UnifyValid, messageService, _) {
        "use strict";
        var configShellCtrl = ["$scope", "$compile", "camel", "appUtilService",
            function ($scope, $compile, camel, appUtilService) {
                ...
                var i18n  = $scope.i18n;
                $scope.shareShellId = $("#applyApp").scope().shareShellId;
                $scope.selShellData = $("#applyApp_configApp_configShellWin").widget().option("selShellData");
                ...
                $scope.ok = {
                    "id": "app_apply_configApp_configShell_ok",
                    "text": i18n.common_term_confirm_label,
                    "click": function () {
                        clearAllValidate();
                        var valid = UnifyValid.FormValid($("#app_apply_configSoftware_configShellId"));
                        if (!valid) {
                            return;
                        }
                        ...
                        saveShellData();
                        $("#applyApp_configApp_configShellWin").widget().destroy();
                    }
                };

                $scope.cancel = {
                    "id": "app_apply_configApp_configShell_cancel",
        ...
        var configParam = angular.module("app.apply.configShell", ['framework', 'ngSanitize']);
        configParam.controller("app.apply.configShell.ctrl", configShellCtrl);
