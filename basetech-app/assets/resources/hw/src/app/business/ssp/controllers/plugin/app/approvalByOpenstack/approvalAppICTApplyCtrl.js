...
define(["tiny-lib/jquery",
    ...
    "fixtures/appFixture"
], function ($, angular, _,Window, UnifyValid, http, messageService,tipMessageService, validatorService, exceptionService, capacityService, cloudInfraService, catalogService, orderService, commonService,appCommonService, timeCommonService,networkService,desigerService,constants,sspException) {
    "use strict";
        ...
        function ($scope,$compile, $state, $stateParams,appUtilService, $q, camel, exception) {
            ...
            $scope.base = {
                ...
                "location": {
                    ...
                    require: true,
                    "id": "approvalAppICT-chooseLocation",
                    "width": 180,
                ...
                expireTime: {
                    ...
                    "id": "serviceapprovalAppICTExpireTime",
                    "width": "178",
                    "require": true,
                    "disable": false,
                    "type": "datetime",
                ...
                neverExpire: {
                    "id": "serviceapprovalAppICTNeverExpire",
                    "checked": false,
            ...
            $scope.info = {
                "commonParams": {
                    "id": "approval-app-ict-commonParams",
                    ...
                    "renderRow": function (nRow, aData, iDataIndex) {
                        ...
                        paramInputScope.disable = (action === "view" || (action === "approval" && aData.inputType !== "2")) ? true : false;
                        ...
                        paramInputScope.change = function () {
                            var paramValue = $("#approvalAppInputCommonParams" + iDataIndex).widget().getValue();
                        ...
                        if ((aData.rType === "az") || (aData.rType === "network") || (aData.rType === "image") || (aData.rType === "instanceType")) {
                            paramSelect = "<div><div style='margin-top: 8px;margin-bottom: 8px;'><tiny-button id='btnId' disable='disable' text='btnText' click='btnClick(vmTemplateId)' ></tiny-button></div></div>";
                        } else {
                            paramSelect = "<div style='height: 43px;'></div>";
                        }
                        var paramSelectLink = $compile(paramSelect);
                        var paramSelectScope = $scope.$new();
                        paramSelectScope.btnId = "approvalAppInputCommonParamsBtn" + iDataIndex;
                        paramSelectScope.btnText = i18n.common_term_choose_label;
                        paramSelectScope.disable = (action === "view" || (action === "approval" && aData.inputType !== "2")) ? true : false;
                        paramSelectScope.btnClick = function () {
                            var rType = aData.rType;
                            var cloudInfraId = $scope.detail.param.lock.locationLock === '2' ?  $("#" + $scope.base.location.id).widget().getSelectedId() : $scope.params.cloudInfraId;
                            var vpcId = $scope.detail.param.lock.vpcLock === '2' ? $("#" + $scope.base.vpc.id).widget().getSelectedId() : $scope.params.selVpcId;
                            var options = {
                                "winId": "approvalByOpenstack_configParamWin",
                                "resourceType": rType,
                            ...
                            if ("az" === rType) {
                                options.content = "app/business/ssp/views/plugin/app/approvalByOpenstack/selAvailableZone.html";
                            } else if ("network" === rType) {
                                options.content = "app/business/ssp/views/plugin/app/approvalByOpenstack/selNetwork.html";
                            } else if ("image" === rType) {
                                options.content = "app/business/ssp/views/plugin/app/approvalByOpenstack/selImage.html";
                            } else if ("instanceType" === rType) {
                                options.content = "app/business/ssp/views/plugin/app/approvalByOpenstack/selInstanceType.html";
                            } else {
            ...
            $scope.approvalResult = {
                ...
                "require": "true",
                "id": "serviceApprovalAppICTResult",
                "spacing": {
                    "width": "50px",
                    "height": "30px"
                },
            ...
            $scope.approvalOpinion = {
                ...
                "id": "serviceApprovalAppICTOpinion",
                "type": "multi",
                "width": "644",
                "height": "60",
                "value": "",
            ...
            $scope.cancelBtn = {
                "id": "serviceapprovalAppICTCancelBtn",
            ...
            $scope.clickAreaHeading = function (id) {
                ...
                if (head.hasClass("collapse")) {
                    if (id === "sspApprovalAppCommonParams" && action === "approval") {
                        if (!saveCommonParams()) {
                            return false;
                        }
                    }
            ...
            $scope.operate = {
                ...
                "queryOrderDetail": function () {
                    ...
                    deferred.then(function (data) {
                        ...
                        $scope.params.applyAppData = JSON.parse($.base64.decode($scope.orderDetail.param.appData, true));
            ...
            function saveCommonParams () {
                var valid = UnifyValid.FormValid($("#approvalAppICT"));
                if (!valid) {
                    return false;
                }
                //校验所有分页的params均是否已填
            ...
            function parseAppTemplate(jsonObj) {
                ...
                var applyParams = $scope.params.applyAppData.commonParams.allData;
                _.each(params, function(item,index){
                    if (item.inputType === "2") {
                        var flag = false;
                        _.each(applyParams, function(item1, index1){
                            if (item.name === item1.name) {
                                flag = true;
                            }
                        });
                        if (!flag) {
                            applyParams.push(item);
                        }
                    }
                });
                $scope.commonParams.allData = applyParams;
            ...
            function checkAllCommonParam() {
                ...
                for (var i = 0; i < $scope.commonParams.allData.length; i++) {
                    //锁定的情况下不用校验
                    if ($scope.commonParams.allData[i].inputType !== "2") {
                        continue;
                    }
