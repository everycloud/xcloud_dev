...
define(['tiny-lib/jquery',
    ...
    function ($, angular, sprintf, ngSanitize, keyIDI18n, http, Window, _, UnifyValid, constants, messageService) {
        "use strict";
        var configParamCtrl = ["$scope", "$compile", "camel", "appUtilService",
            function ($scope, $compile, camel, appUtilService) {
                $scope.allInputIds = [];
                $scope.shareSoftId = $("#approvalApp").scope().shareSoftId;
                $scope.selSoftData = $("#approvalApp_configApp_configSoftwareWin").widget().option("selSoftData") || {};
                ...
                $scope.ok = {
                    "id": "app_approval_configSoftware_configSoft_ok",
                    "text": i18n.common_term_confirm_label,
                    "click": function () {
                        clearAllValidate();
                        var valid = UnifyValid.FormValid($("#app_approval_configSoftware_popwin"));
                        if (!valid) {
                            return;
                        }
                        ...
                        saveSoftData();
                        $("#approvalApp_configApp_configSoftwareWin").widget().destroy();
                    }
                };

                $scope.cancel = {
                    "id": "app_approval_configSoftware_configSoft_cancel",
                ...
                function getOptionByNameAndType(type, name) {
                    ...
                    // shell分支
                        ...
                        options = $("#app_approval_configSoftware_configShellId").scope().install.options;
                    } else {
                        options = [];
                    }
        ...
        var configParamModule = angular.module("app.approval.configSoftware", ["framework", "ngSanitize"]);
        configParamModule.controller("app.approval.configSoftware.ctrl", configParamCtrl);
