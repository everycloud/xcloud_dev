...
define(['tiny-lib/jquery',
    ...
    function ($, sprintf, ngSanitize, keyIDI18n, angular, http, Window, _, UnifyValid, constants, messageService) {
        "use strict";
        var configParamCtrl = ["$scope", "$compile", "camel", "appUtilService",
            function ($scope, $compile, camel, appUtilService) {
                $scope.allInputIds = [];
                $scope.shareSoftId = $("#applyApp").scope().shareSoftId;
                $scope.selSoftData = $("#applyApp_configApp_configSoftwareWin").widget().option("selSoftData") || {};
                ...
                $scope.ok = {
                    "id": "app_apply_configSoftware_configSoft_ok",
                    "text": i18n.common_term_confirm_label,
                    "click": function () {
                        clearAllValidate();
                        var valid = UnifyValid.FormValid($("#app_apply_configSoftware_popwin"));
                        if (!valid) {
                            return;
                        }
                        ...
                        saveSoftData();
                        $("#applyApp_configApp_configSoftwareWin").widget().destroy();
                    }
                };

                $scope.cancel = {
                    "id": "app_apply_configSoftware_configSoft_cancel",
                ...
                function popupConfig(type, name, optionId) {
                    ...
                    var options = {
                        "winId": "applyApp_configApp_configSoftware_associate",
                        "configSoftOrShell": configSoftOrShell,
                        "title": i18n.common_term_associate_button,
                        "width": "400px",
                        "height": "250px",
                        "content-type": "url",
                        "content": "app/business/ssp/views/plugin/app/submit/configParamAssociate.html",
                        "buttons": null
                    };
                ...
                function getOptionByNameAndType(type, name) {
                    ...
                    // shell分支
                        ...
                        options = $("#app_apply_configSoftware_configShellId").scope().install.options;
                    } else {
                        options = [];
                    }
        ...
        var configParamModule = angular.module("app.apply.configSoftware", ['framework', 'ngSanitize']);
        configParamModule.controller("app.apply.configSoftware.ctrl", configParamCtrl);
