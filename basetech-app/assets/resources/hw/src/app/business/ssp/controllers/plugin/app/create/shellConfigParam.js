    ...
    function ($, sprintf, ngSanitize, keyIDI18n, angular, http, constants, Window, UnifyValid, messageService, _) {
        ...
        var configShellCtrl = ["$scope", "$compile", "camel", "appUtilService",
            function ($scope, $compile, camel, appUtilService) {
                ...
                var i18n  = $scope.i18n;
                $scope.shareShellId = $("#createAppConfigApp").scope().shareShellId;
                $scope.selShellData = $("#createApp_configApp_configShellWin").widget().option("selShellData");
                ...
                $scope.ok = {
                    "id": "app_create_configApp_configShell_ok",
                    "text": i18n.common_term_confirm_label,
                    "click": function () {
                        clearAllValidate();
                        var valid = UnifyValid.FormValid($("#app_create_configSoftware_configShellId"));
                        if (!valid) {
                            return;
                        }
                        ...
                        saveShellData();
                        $("#createApp_configApp_configShellWin").widget().destroy();
                    }
                };

                $scope.cancel = {
                    "id": "app_create_configApp_configShell_cancel",
                ...
                function popupConfig(type, name) {
                    ...
                    var options = {
                        ...
                        "height": "250px",
                        "content-type": "url",
                        "content": "app/business/ssp/views/plugin/app/create/configParamAssociate.html",
                        "buttons": null
                    };
        ...
        var configParam = angular.module("app.create.configShell", ['framework', 'ngSanitize']);
        configParam.controller("app.create.configShell.ctrl", configShellCtrl);
