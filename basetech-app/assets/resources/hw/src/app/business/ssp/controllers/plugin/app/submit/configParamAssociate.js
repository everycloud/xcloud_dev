...
define(['tiny-lib/jquery',
    ...
    function ($, sprintf, ngSanitize, keyIDI18n, angular, http, Window, _, UnifyValid, constants) {
        "use strict";
        var configParamCtrl = ["$scope", "$compile", "camel",
            function ($scope, $compile, camel) {
                $scope.vmNameMap = null;
                $scope.vmNicsMap = null;
                $scope.shareAssociate = $("#applyApp_configApp_configSoftware_associate").widget().option("configSoftOrShell") || {};
                keyIDI18n.sprintf = sprintf.sprintf;
                $scope.i18n = keyIDI18n;
                var i18n  = $scope.i18n;
                $scope.vm = {
                    "id": "apply_config_softAssociate_vm",
                    ...
                    "values": [],
                    "change": function () {
                        var selVmId = $("#apply_config_softAssociate_vm").widget().getSelectedId();
                        var nics = $scope.vmNicsMap[selVmId];
                        var nicSelValues = [];
                        $("#apply_config_softAssociate_nic").widget().opChecked();
                        if (nics) {
                            var tmpNicSel;
                ...
                $scope.nic = {
                    "id": "apply_config_softAssociate_nic",
                    ...
                    "values": [],
                    "change": function () {
                        var selNicId = $("#apply_config_softAssociate_nic").widget().getSelectedId();
                ...
                $scope.nextBtn = {
                    "id": "applyApp-baseInfo-nextBtn",
                    "text": i18n.common_term_ok_button,
                    "click": function () {
                        var valid = UnifyValid.FormValid($("#app_apply_configSoftware_associatePop"));
                        if (!valid) {
                            return;
                        }
                        ...
                        var associateVmId = $("#apply_config_softAssociate_vm").widget().getSelectedId();
                        var associateVmLabel = $("#apply_config_softAssociate_vm").widget().getSelectedLabel();
                        var associateNicId = $("#apply_config_softAssociate_nic").widget().getSelectedId();
                        var associateNicLabel = $("#apply_config_softAssociate_nic").widget().getSelectedLabel();
                        var type = $scope.shareAssociate.type;
                        var name = $scope.shareAssociate.name;
                        var configScope = $("#app_apply_configSoftware_popwin").scope();
                        ...
                        if ("shellInstall" !== type) {
                            ...
                            var shellConfigScope = $("#app_apply_configSoftware_configShellId").scope();
                        ...
                        $("#applyApp_configApp_configSoftware_associate").widget().destroy();
                    }
                };

                $scope.cancelBtn = {
                    "id": "applyApp-baseInfo-cancel",
                ...
                function getOptionByNameAndType(type, optionName) {
                    ...
                    var options;
                    if ("shellInstall" === type) {
                        scope = $("#app_apply_configSoftware_configShellId").scope();
                        options = scope.install.options;
                    } else {
                        scope = $("#app_apply_configSoftware_popwin").scope();
                        if ("install" === type) {
                            options = scope.install.options;
                ...
                function init() {
                    $scope.vmNameMap = {};
                    var allVmNameMap = $("#applyApp").scope().tmp.vmNameMap;
                    ...
                    $scope.vmNicsMap = $("#applyApp").scope().tmp.vmNicsMap;
                    var vmSelValues = [];
                    var tmpVmSel = null;
                    _.each($scope.vmNameMap, function (item, index) {
                        if ( $("#applyApp").scope().tmp.vmTempInstanceMap[index]) {
                            tmpVmSel = {
                                "selectId": index,
                                "label": item
                            };
        ...
        var configParamModule = angular.module("app.apply.configSoftware.associate", ['framework', 'ngSanitize']);
        configParamModule.controller("app.apply.configSoftware.associate.ctrl", configParamCtrl);
