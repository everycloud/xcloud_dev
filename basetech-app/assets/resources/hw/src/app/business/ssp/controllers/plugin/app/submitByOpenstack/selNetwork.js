    ...
    function (sprintf, $, encoder, angular, ngSanitize, keyIDI18n, http, Window, _, UnifyValid, networkService) {
        ...
        var configParamCtrl = ["$scope", "$compile", "camel", "exception", "appCommonData", "$q",
            function ($scope, $compile, camel, exception, appCommonData, $q) {
                ...
                $scope.resPoolId = $("#applyByOpenstack_configParamWin").widget().option("resPoolId");
                $scope.vpcId = $("#applyByOpenstack_configParamWin").widget().option("vpcId");
                $scope.vpcName = $("#applyByOpenstack_configParamWin").widget().option("vpcName");
                $scope.curParamTableData = $("#applyByOpenstack_configParamWin").widget().option("curParamTableData");

                // ict场景 需要根据网络类型过滤网络
                $scope.ictNetType = {
                    "label": i18n.vpc_term_netType_label + ":",
                    "id": "apply-app-ictNetType",
                    "require": true,
                    "width": 220,
                    "values": [
                        {
                ...
                $scope.networkTable = {
                    ...
                    "renderRow": function (nRow, aData, iDataIndex) {
                        ...
                        selBoxScope.name = "appApplyAppChooseNetworkRadio";
                        selBoxScope.id = "appApplyAppChooseNetworkRadio" + iDataIndex;
                ...
                $scope.nextBtn = {
                    ...
                    "click": function () {
                        $scope.curParamTableData.value = $("#apply-app-subnetselect").widget().getSelectedLabel();
                        $scope.curParamTableData.selectId = $("#apply-app-subnetselect").widget().getSelectedId();
                        $("#applyByOpenstack_configParamWin").widget().destroy();
                    }
                };
                    ...
                    if ($scope.ictNetworkType === "VPCNET") {
                        ...
                        deferred.then(function (data) {
                            ...
                            _.each(data.networks, function (item) {
                                item.statusUI = appCommonData.getNetworkStatus(item.status);
                            });
        ...
        var selImageModule = angular.module("ssp.applyByOpenstack.selNetwork", ['framework','ngSanitize']);
        selImageModule.controller("ssp.applyByOpenstack.selNetwork.ctrl", configParamCtrl);
