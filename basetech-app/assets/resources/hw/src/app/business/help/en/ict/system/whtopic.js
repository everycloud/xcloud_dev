...
{
    var strHomeName = "%%%WH_LNG_Home%%%"
    
	if(strHomeName.substring( 0,2)=="%%")
	{
		strHomeName = strHome;
	}
	...
	if(i == 0)
	{
	    ...
	    if(styleInfo == "breadcrumbs")
		    strTrail = "<a class=\""+ styleInfo + "\"" + " href=\"" + strHomePath + "\">" + strHomeName + "</a> " + ((strHome == "")? "":separator) + " ";
	    else
	        strTrail = "<a style=\""+ styleInfo + "\"" + " href=\"" + strHomePath + "\">" + strHomeName + "</a> " + ((strHome == "")? "":separator) + " ";
...
function DomTextNode( a_Node, a_nFrom )
{
	...
	this.doHighlight = function( a_aRanges, a_nStart )
	{
		...
		for ( var i = 0; i < this.aClosedRanges.length; i++ )
		{
			...
			var  tmpStr = strText.substring( nLastStart, this.aClosedRanges[i].nStart - this.nFrom + lastNewLineoffset);
			var newLineCount;
			if (gbNav6)
			{
				newLineCount = tmpStr.match(/[\n\t]/g);
			}
			else
			{
				newLineCount = tmpStr.match(/[\r\t]/g);
			}

			if (newLineCount != null)
			{
				lastNewLineoffset = lastNewLineoffset + newLineCount.length;
			}
			
			if (this.node.parentNode.nodeName == 'PRE')
			{
				strHTML += _textToHtml(strText.substring( nLastStart, this.aClosedRanges[i].nStart - this.nFrom + lastNewLineoffset));
			}
			else
			{
				strHTML += _textToHtml_nonbsp(strText.substring( nLastStart, this.aClosedRanges[i].nStart - this.nFrom + lastNewLineoffset));
			}
			strHTML += s_strHlStart;
			
			if (this.node.parentNode.nodeName == 'PRE')
			{
				strHTML += _textToHtml(strText.substring( this.aClosedRanges[i].nStart - this.nFrom + lastNewLineoffset,
											  this.aClosedRanges[i].nEnd - this.nFrom + lastNewLineoffset));
			}
			else
			{
				strHTML += _textToHtml_nonbsp(strText.substring( this.aClosedRanges[i].nStart - this.nFrom + lastNewLineoffset,
											  this.aClosedRanges[i].nEnd - this.nFrom + lastNewLineoffset));
			}

			strHTML += s_strHlEnd;

			nLastStart = this.aClosedRanges[i].nEnd - this.nFrom + lastNewLineoffset;
		}
		if (this.node.parentNode.nodeName == 'PRE')
		{
			strHTML += _textToHtml(strText.substr( nLastStart ));
		}
		else
		{
		...
		if (this.node.parentNode.nodeName == 'PRE' && gbIE)
		{
			var re=new RegExp("\r","g");
			var strHTMLwithBR = strHTML.replace(re, "<BR/>");
			spanElement.innerHTML = strHTMLwithBR;
		}
		else
		{
...
function highlightSearch()
{
	...
	StartHighLightSearch(chaifenInput(strTerms));	

}


		
function chaifenInput(strTerms)
{
		if (typeof(strTerms) == "undefined")
		{
			return "";
		}
        var sbStr="";
        var strs = strTerms.split(""); 
	    for(var i = 0; i < strs.length; i++) { 
		   if(escape(strs[i]).indexOf("%u")<0){      
			  if(escape(strs[i+1]).indexOf("%u")<0){
				  sbStr+=strs[i]
			}else{
				sbStr+=strs[i]+" "
			      }
