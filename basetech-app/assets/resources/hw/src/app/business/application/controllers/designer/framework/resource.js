(function () {
    ...
    SoftwareDefine.prototype.updateCommands = function (templatedef) {
        ...
        for (var key in properties) {
            ...
            var cmdParams = command.match(/(?!\{)[^\{\}]+(?=\})/g); //支持 ping  {ip=192.168.1.7} {port}
            if (!cmdParams) {
                continue;
            }
            for (var i = 0; i < cmdParams.length; i++) {
                ...
                var value = null;
                var exp = null; //正則表達式
                if (/uninstall/.test(key)) { 
                    item["value"] = this.getUninstallParam(item["name"]); 
                } else if (/install/.test(key)) { 
                    item["value"] = this.getInstallParam(item["name"]); 
                } else if (/start/.test(key)) { 
                    item["value"] = this.getStartParam(item["name"]);
    ...
    ConnectionDefine = function (options) {
        ...
        // 是否为删除操作
        this.opAutoDel = null;
        // vm-> portgroup: 内容为: vm.nicName -- pg.id
        // vm-> vm: 内容为: targetVm.nicName -- [{sid: xxx, group: yyy, cmd: kkk, status: "on/off"}, ..]
    ...
    TemplateDefine = function (options) {
        ...
        this.rebuildVmtemplates = function (resource, jsonRes, bodyObj, vmConnectionList, isBuildGraph, coreGraph) {
            ...
            if (isBuildGraph) {
                ...
                if (hasScalingGroup) {
                    ...
                    var scalinggroup = TemplateDefine.createResource(this, [], "ScalingGroup", jsonResources[key].Name, key);
                    this.rebuildScalingGroups(this, scalinggroup, jsonResources[key]);
                    vmTemplateCell.scalinggroup = scalinggroup;
         ...
         * @return true/false
         */
        this.rebuildScalingGroups = function (template, resource, jsonRes) {
            // 判断绘图对象是否存在，如果不存在，则无法进行绘图，直接返回
            resource.setName(jsonRes.Properties.Name);
    ...
    TemplateDefine.prototype.setTemplateName = function (name) {
        this.properties["templateName"] = name;
    };
     ...
     *
     * @param templateBody
     * @param templateBody 反解析需要的json字符串/也可以是已经生成好的json对象
     * @param isBuildGraph
     *                    是否需要画图。
     *                    如果只是解析模板时，isBuildGraph为false，如果要在画布上画图，则为true
     * @return true/false 表示是否进行了反解析
     */
    TemplateDefine.prototype.rebuildGraph = function (templateBody, isBuildGraph, coreGraph) {
        ...
        try {
            bodyObj = JSON.parse(templateBody);
        } catch (e) {
        ...
        this.setTemplateName(bodyObj.TemplateName);
