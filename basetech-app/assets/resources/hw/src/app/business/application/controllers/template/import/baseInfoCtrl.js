...
define(['jquery',
    ...
    "upload/jquery-form",
    'tiny-widgets/FileUpload',
    'tiny-directives/FileUpload'],
    function ($, angular, UnifyValid, Message, mask, Exception) {
        "use strict";

        var baseInfoCtrl = ["$scope", "$window", "$state", "$timeout","$stateParams",
            function ($scope, $window, $state, $timeout, $stateParams) {

                var user = $("html").scope().user;
                var exceptionService = new Exception();
                $scope.openstack = ($scope.user.cloudType === "ICT" ? true : false);
                ...
                $scope.templateName = {
                    ...
                    "selectError": function (event, file, errorMsg) {
                        ...
                        $("#" + $scope.templateName.idSingle).widget().empty();
                    },
                    "method": "post",
                    ...
                    "beforeSubmit": function (event, file) {
                        ...
                        $("#" + $scope.templateName.idSingle).widget().addFormData({
                            ...
                            "resPoolType": (user.cloudType !== "ICT" ? "FusionManager" : "OpenStack"),
                ...
                $scope.uploadHandle = function () {
                    ...
                    $("#" + $scope.templateName.idSingle + " form").attr("action", "/goku/rest/v1.5/" + user.vdcId + "/apptemplates/actions");
