...
define(['jquery',
    ...
    var transferVmCtrl = ["$scope", "validator", "camel", function ($scope, validator, camel) {
        ...
        function transfer(selectedUser) {
            var params = {
                allocate: {
                    "volumeIds": [options.selectedDisk],
                    "userId": selectedUser
                }
            };
            var deferred = camel.post({
    ...
    var transferVmModule = angular.module("ecs.disk.transferDisk", ["ng"]);
    transferVmModule.service("validator", validatorService);
    transferVmModule.service("camel", httpService);
    transferVmModule.controller("ecs.disk.transferDisk.ctrl", transferVmCtrl);
