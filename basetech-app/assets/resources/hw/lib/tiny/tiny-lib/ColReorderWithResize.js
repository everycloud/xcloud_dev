...
ColReorder.prototype = {
    ...
    if ($(nTh).css('cursor') == 'col-resize') {         
      ...
      that.dom.resize = null;
		...
		//如果存在弹出框就改变document
    	d = $('.modal',top.document).size() > 0 ? top.document : document;
		
		/* Add event handlers to the document */
		$(d).bind( 'mousemove.ColReorder', function (e) {
			that._fnMouseMove.call( that, e, i); 
		} );
		
		$(d).bind( 'mouseup.ColReorder', function (e) {
	...
	"_fnMouseMove": function ( e, colResized )
	{
		...
		if (this.dom.resize) {       
		  ...
		  if ((this.s.mouse.startWidth + moveLength) > 5)
		  ...
		  if($('div.dataTables_scrollBody', this.s.dt.nTableWrapper) != null){
			...
			if($('div.dataTables_scrollBody', this.s.dt.nTableWrapper).length > 0){
				...
				if (moveLength != 0 && !scrollXEnabled && (this.s.mouse.nextStartWidth - moveLength) > 5){
	...
	"_fnMouseUp": function ( e, colResized)
	{
		...
		$(d).unbind( 'mousemove.ColReorder' );
		$(d).unbind( 'mouseup.ColReorder' );

		if ( this.dom.drag !== null )
		{
			/* Remove the guide elements */
			d.body.removeChild( this.dom.drag );
			d.body.removeChild( this.dom.pointer );
	...
	"_fnCreateDragNode": function ()
	{
		...
		//如果存在弹出框就改变document
    	d = $('.modal',top.document).size() > 0 ? top.document : document;
		this.dom.pointer = d.createElement( 'div' );
		...
		d.body.appendChild( this.dom.pointer );
		d.body.appendChild( this.dom.drag );
	},
